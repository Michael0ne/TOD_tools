...
class BuiltinType : public EntityType
{
    ...
    typedef void (BuiltinType::* BuiltinTypeMemberFunction)(void*);
    ...
    struct BuiltinMember
    {
        ...
        inline BuiltinMember(DataType* returntype, Getter getter, Setter setter, const char* const prototype, const char* const dummy)
        {
        ...
        ~BuiltinMember()
        {
            MESSAGE_CLASS_DESTROYED(BuiltinMember);
        }
    };
    ...
    {
        String  m_Prototype;
        BuiltinTypeMemberFunction m_Handler;
        String  m_Name;
        
        inline BuiltinHandler(const char* _prot, BuiltinTypeMemberFunction _handler, const char* _name)
        {
            MESSAGE_CLASS_CREATED(BuiltinHandler);
        ...
        ~BuiltinHandler()
        {
            MESSAGE_CLASS_DESTROYED(BuiltinHandler);
        }
    };

private:
    std::vector<BuiltinHandler> m_HandlersList;
    std::map<int, BuiltinMember>   m_MembersMap;
    int       field_94;

    BuiltinType(); // @486D00
    virtual ~BuiltinType(); //  @489540
    ...
    void RegisterHandler(
        const char* handlerSignature,
        BuiltinTypeMemberFunction handlerPtr,
        const char* handlerHelperMessage); // @486430
...
public:
    ...
    const unsigned int  GetGameTimeMs() const; //  @487FA0
    const unsigned int  GetRealTimeMs() const;  //  @487FB0
    const float         GetGameTime() const;    //  @487FC0
    const float         GetRealTime() const;    //  @487FE0
    const float         GetTimePassed() const;  //  @488000
    const float         GetRealTimePassed() const;  //  @488010
    const float         GetPI() const;  //  @487E40
    void    GetInVector(Vector4f& outVec) const;    //  @487E50
    void    GetOutVector(Vector4f& outVec) const;   //  @487E80
    void    GetRightVector(Vector4f& outVec) const; //  @487EB0
    void    GetLeftVector(Vector4f& outVec) const;  //  @487EE0
    void    GetUpVector(Vector4f& outVec) const;    //  @487F10
    void    GetDownVector(Vector4f& outVec) const;  //  @487DF0
    void    GetZeroVector(Vector4f& outVec) const;  //  @487F70
    bool    GetWarnDelayedException() const;    //  @488020
    void    SetWarnDelayedException(const bool warn);   //  @488030
    int     GetSoundRenderer() const;   //  @485040
