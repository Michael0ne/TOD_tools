...
namespace Audio
{
	...
	void StreamedSoundBuffers::InitDieselPower()
	{
		...
		m_pDieselPower = DieselPower::CallFactory(1, 2, 1, 3.0f, g_Window->m_WindowHandle, 1, 0, 0);
	...
	void StreamedSoundBuffers::InitDirectSound(char channels, int sampleRate)
	{
		...
		if (FAILED(m_pDirectSound->SetCooperativeLevel(g_Window->m_WindowHandle, SoundPriorityLevels[2])))
		{
			for (; soundPriorityLevel < 5; soundPriorityLevel++)
				if (SUCCEEDED(m_pDirectSound->SetCooperativeLevel(g_Window->m_WindowHandle, SoundPriorityLevels[soundPriorityLevel])))
					break;
